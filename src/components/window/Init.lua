...
return function(Config)
    ...
    if Window.Folder then
        if not isfolder("ArtUI/" .. Window.Folder) then
            makefolder("ArtUI/" .. Window.Folder)
        end
        if not isfolder("ArtUI/" .. Window.Folder .. "/assets") then
            makefolder("ArtUI/" .. Window.Folder .. "/assets")
        end
    ...
    if typeof(Window.Background) == "string" and BGVideo then
        ...
        if string.find(BGVideo, "http") then
            ...
            if not isfile(videoPath) then
                ...
                if not success then
                    warn("[ ArtUI.Window.Background ] Failed to download video: " .. tostring(result))
                    return
                end
            end
            ...
            if not success then
                warn("[ ArtUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
                return
            end
    ...
    Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
        local LeftWidth = 0
        local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X / Config.ArtUI.UIScale
        -- if WindowTitle and WindowAuthor then
        --     LeftWidth = math.max(WindowTitle.TextBounds.X / Config.ArtUI.UIScale, WindowAuthor.TextBounds.X / Config.ArtUI.UIScale)
        -- else
        --     LeftWidth = WindowTitle.TextBounds.X / Config.ArtUI.UIScale
        -- end
        LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.ArtUI.UIScale
        if Window.Topbar.ButtonsType ~= "Default" then
            LeftWidth = LeftWidth + RightWidth + Window.UIPadding - 4
        end
        -- if WindowIcon then
        --     LeftWidth = LeftWidth + (Window.IconSize / Config.ArtUI.UIScale) + (Window.UIPadding / Config.ArtUI.UIScale) + (4 / Config.ArtUI.UIScale)
        -- end
        Window.UIElements.Main.Main.Topbar.Center.Position = UDim2.new(
            0,
            LeftWidth + (Window.UIPadding / Config.ArtUI.UIScale),
            0.5,
            0
        )
        Window.UIElements.Main.Main.Topbar.Center.Size = UDim2.new(
            1,
            - LeftWidth - RightWidth - ((Window.UIPadding * 2) / Config.ArtUI.UIScale),
            1,
            0
        )
    end)
    ...
    if Window.Topbar.ButtonsType ~= "Default" then
        Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"), function()
            Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(0,(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X/Config.ArtUI.UIScale) + Window.UIPadding - 4,0,0)
        end)
    end
    ...
    if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
        ...
        Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
            ...
            ImageTransparency = Window.Transparent and Config.ArtUI.TransparencyValue or 0
        }, {
            BackgroundGradient
        })
    end
    ...
    function Window:SetBackgroundTransparency(v)
        ...
        Config.ArtUI.TransparencyValue = rounded
    ...
    if Config.ArtUI.UseAcrylic then
    ...
    function Window:Open()
        task.spawn(function()
            ...
            task.spawn(function()
                ...
                Config.ArtUI:ToggleAcrylic(true)
            end)
        end)
    end
    function Window:Close()
        ...
        Config.ArtUI:ToggleAcrylic(false)
        ...
        function Close:Destroy()
            task.spawn(function()
                ...
                task.wait(0.4)
                Config.ArtUI.ScreenGui:Destroy()
                Config.ArtUI.NotificationGui:Destroy()
                Config.ArtUI.DropdownGui:Destroy()
                Config.ArtUI.TooltipGui:Destroy()
    ...
    function Window:ToggleTransparency(Value)
        ...
        Config.ArtUI.Transparent = Value
        
        Window.UIElements.Main.Background.ImageTransparency = Value and Config.ArtUI.TransparencyValue or 0
        -- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.ArtUI.TransparencyValue or 0
    ...
    function Window:GetUIScale(v)
        return Config.ArtUI.UIScale 
    end
    
    function Window:SetUIScale(v)
        Config.ArtUI.UIScale = v
        Tween(Config.ArtUI.UIScaleObj, .2, {Scale = v}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        return Window
    end
    ...
    local TabModule = TabModuleMain.Init(Window, Config.ArtUI, Config.ArtUI.TooltipGui)
    ...
    function Window:Tab(TabConfig)
        ...
        return TabModule.New(TabConfig, Config.ArtUI.UIScale)
    end
    ...
    function Window:Section(SectionConfig)
        return SectionModule.New(SectionConfig, Window.UIElements.SideBar.Frame, Window.Folder, Config.ArtUI.UIScale, Window)
    end
    ...
    function Window:Dialog(DialogConfig)
        ...
        local function CheckButtonsOverflow()
            ...
            local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.ArtUI.UIScale
            local parentWidth = ButtonsContent.AbsoluteSize.X / Config.ArtUI.UIScale
            ...
            if totalWidth > parentWidth then
                ...
                if availableSpace > 0 then
                    ...
                    for _, button in ipairs(Buttons) do
                        local buttonWidth = button.AbsoluteSize.X / Config.ArtUI.UIScale
