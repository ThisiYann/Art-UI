...
function Creator.Init(ArtUITable)
    ArtUI = ArtUITable
end
...
function Creator.SafeCallback(Function, ...)
    ...
    if not Success then
        if ArtUI and ArtUI.Window and ArtUI.Window.Debug then
            local _, i = Event:find(":%d+: ")
        
            warn("[ ArtUI: DEBUG Mode ] " .. Event)
...
function Creator.Gradient(stops, props)
    if ArtUI and ArtUI.Gradient then
        return ArtUI:Gradient(stops, props)
    end
...
function Creator.UpdateTheme(TargetObject, isTween)
    local function ApplyTheme(objData)
        for Property, ColorKey in pairs(objData.Properties or {}) do
            ...
            if value ~= nil then
                if typeof(value) == "Color3" then
                    local gradient = objData.Object:FindFirstChild("ArtUIGradient")
                    if gradient then
                        gradient:Destroy()
                    end
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
    ...
    elseif string.find(Img,"http") then
        local FileName = "ArtUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
        local success, response = pcall(function()
            task.spawn(function()
                ...
                if assetSuccess then
                    ...
                    warn(string.format("[ ArtUI.Creator ] Failed to load custom asset '%s': %s", FileName, tostring(asset)))
                    ImageFrame:Destroy()
        ...
        if not success then
            warn("[ ArtUI.Creator ]  '" .. identifyexecutor() .. "' doesnt support the URL Images. Error: " .. response)
