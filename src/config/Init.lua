...
function ConfigManager:Init(WindowTable)
    ...
    ConfigManager.Path = "ArtUI/" .. tostring(ConfigManager.Folder) .. "/config/"
    
    if not isfolder("ArtUI/" .. ConfigManager.Folder) then
        makefolder("ArtUI/" .. ConfigManager.Folder)
        if not isfolder("ArtUI/" .. ConfigManager.Folder .. "/config/") then
            makefolder("ArtUI/" .. ConfigManager.Folder .. "/config/")
        end
    end
    ...
    if isfile(ConfigModule.Path) then
        ...
        if success and configData and configData.__autoload then
            ...
            task.spawn(function()
                ...
                if success then
                    if Window.Debug then print("[ ArtUI.ConfigManager ] AutoLoaded config: " .. configFilename) end
                else
                    warn("[ ArtUI.ConfigManager ] Failed to AutoLoad config: " .. configFilename .. " - " .. tostring(result))
                end
            end)
        end
    end
