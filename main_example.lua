...
local Window = ArtUI:CreateWindow({
    ...
    --[[
    KeySystem = {
        Title = "Key System Example  |  ArtUI Example",
...
The project is primarily written in Lua (Luau), the scripting language used in Roblox.]],
    ...
    AboutTab:Button({
        Title = "Export ArtUI JSON (copy)",
        Color = Color3.fromHex("#a2ff30"),
        Justify = "Center",
        IconAlign = "Left",
        Icon = "", -- removing icon
        Callback = function()
            tableToClipboard(ArtUI)
            ArtUI:Notify({
                Title = "ArtUI JSON",
...
do
    ...
    ButtonTab:Button({
        ...
        Callback = function()
            ArtUI:Notify({
                Title = "Hello",
                Content = "Welcome to the ArtUI Example!",
...
do
    ...
    local Response = ArtUI.cloneref(game:GetService("HttpService")):JSONDecode(ArtUI.Creator.Request({
        Url = DiscordAPI,
        Method = "GET",
        Headers = {
            ["User-Agent"] = "ArtUI/Example",
...
do
    ...
    ArtUI.Creator.AddIcons("fluency",    NebulaIcons.Fluency)
    --               ^ Icon name          ^ Table of Icons
    
    -- You can also add nebula icons
    ArtUI.Creator.AddIcons("nebula",    NebulaIcons.nebulaIcons)
